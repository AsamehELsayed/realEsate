...
const _sfc_main = /* @__PURE__ */ Object.assign({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`</div><div class="mb-4"><label for="price" class="block text-sm font-medium text-gray-700">Price</label><input id="price" type="number" step="0.01"${ssrRenderAttr("value", unref(form).price)} class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500"${ssrRenderAttr("aria-describedby", unref(form).errors.price ? "price-error" : null)}>`);
      if (unref(form).errors.price) {
        _push(`<div id="price-error" class="text-red-500 text-sm mt-1">${ssrInterpolate(unref(form).errors.price)}</div>`);
      } else {
        _push(`<!---->`);
      }
      ...
      _push(`</div><div class="mb-4"><label for="total" class="block text-sm font-medium text-gray-700">Total Price</label><input id="total" type="text"${ssrRenderAttr("value", total.value)} class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500" readonly></div><button type="submit" class="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 focus:outline-none"> Save Changes </button></form></div>`);
    };
  }
});
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/Pages/Dashboard/Prices/edit.vue");
