<template>
    <div class="w-full flex justify-between items-center mb-6">
      ...
      <!-- <Link :href="route('programs.create')" class="px-6 py-3 bg-indigo-600 text-white font-semibold rounded-md shadow hover:bg-indigo-700 focus:outline-none focus:ring focus:ring-indigo-300">
        + Create
      </Link> -->
    </div>
      ...
      <table class="w-full text-sm text-left text-gray-700 dark:text-gray-400">
        ...
        <tbody>
          <tr v-for="program in programs" :key="program.id" 
            ...
            <td class="px-6 py-4 font-medium text-gray-900 dark:text-white">{{ program.name }}</td>
            <td class="px-6 py-4">{{ program.email }}</td>
            <td class="px-6 py-4">{{ program.phone }}</td>
            <td class="px-6 py-4 flex justify-center gap-2">
           
              <Link :href="route('programs.destroy', program.id)" 
                    method="delete"
